...
class Survey_controller extends CI_Controller {
...

	{
	    ...
	    $data['report_type5'] = $this->survey_model->get_report($perusahaan, "5");
	    $data['report_type6'] = $this->survey_model->get_report($perusahaan, "6");
	...
	public function update_user($id_user) 
	{
		...
		$id_batch = $this->input->post('id-batch');
...

	{
	    ...
	    $user = $this->survey_model->get_user_by_id($id_user);
...

	{
	    ...
	    $this->db->delete('user_survey', array('id' => $id));
...

	{
		...
		$this->session->set_userdata(['id_import_user' => $id_perusahaan]);
	...
	public function user_verification(){
		...
		if($on_batch){
			...
			$startBatch = date('YmdHis',strtotime($on_batch->start));
	
			$endBatch = date('YmdHis',strtotime($on_batch->end));
	
			$dateNow = date("YmdHis");

			if((int)$dateNow<=(int)$endBatch && (int)$dateNow>=(int)$startBatch){
	
				if($on_batch->status != "1"){
					...
					$this->session->set_flashdata('error', 'Maaf, anda sudah melakukan survey');
...

	{
		...
		$data['perusahaan'] = $this->session->userdata("id_perusahaan");
